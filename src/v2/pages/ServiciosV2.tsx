...
function ServiceModal({ service, onClose }: { service: Service; onClose: () => void }) {
  ...
  return (
    <motion.div
      ...
      onClick={onClose}
      style={{ position: 'fixed', inset: 0, zIndex: 9000, background: 'rgba(0,0,0,0.75)', backdropFilter: 'blur(12px)', WebkitBackdropFilter: 'blur(12px)', display: 'flex', alignItems: 'center', justifyContent: 'center', padding: isMobile ? 0 : 24 }}
    >
      <motion.div
        key="card"
        ...
        style={{
          ...
          width: '100%',
          height: isMobile ? '92vh' : 'min(580px, 90vh)',
          display: 'flex',
          flexDirection: isMobile ? 'column' : undefined,
          ...(isMobile ? {} : { gridTemplateColumns: '1fr 1fr', display: 'grid' as any }),
          alignSelf: isMobile ? 'flex-end' : undefined,
        }}
      >
        {/* Imagen */}
        <div style={{ position: 'relative', overflow: 'hidden', height: isMobile ? 220 : undefined, flexShrink: isMobile ? 0 : undefined }}>
        ...
        {/* Contenido */}
        <div style={{ padding: isMobile ? '24px 20px 32px' : '40px 36px', overflowY: 'auto', display: 'flex', flexDirection: 'column', flex: 1 }}>
          ...
          <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 20 }}>
            <h2 style={{ fontFamily: 'Bebas Neue, sans-serif', fontSize: isMobile ? 28 : 'clamp(28px, 3vw, 40px)', letterSpacing: '0.03em', color: '#fff', lineHeight: 1.1 }}>{service.title}</h2>
            <motion.button
          ...
          <p style={{ fontSize: 14, color: grayLight, lineHeight: 1.8, marginBottom: 24 }}>{service.fullDesc}</p>
    ...
    <div style={{ background: bg, color: '#fff', fontFamily: 'Inter, sans-serif' }}>
      ...
      <section ref={heroRef} style={{ position: 'relative', height: '50vh', overflow: 'hidden', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
        ...
        >
          ...
          <h1 style={{ fontFamily: 'Bebas Neue, sans-serif', fontSize: isMobile ? 48 : 'clamp(48px, 8vw, 80px)', letterSpacing: '0.05em', color: '#fff', textAlign: 'center' }}>
            NUESTROS SERVICIOS
          </h1>
        </motion.div>
      </section>
      ...
      <motion.section
        ...
        style={{ padding: isMobile ? '60px 24px 32px' : '80px 48px 40px', maxWidth: 1200, margin: '0 auto', display: 'grid', gridTemplateColumns: isMobile ? '1fr' : '1fr 1fr', gap: isMobile ? 20 : 60, alignItems: 'end' }}
      >
        <div>
          ...
          <motion.h2 variants={fadeUp} style={{ fontFamily: 'Bebas Neue, sans-serif', fontSize: isMobile ? 32 : 'clamp(32px, 4vw, 52px)', letterSpacing: '0.02em', color: '#fff', lineHeight: 1.1 }}>
      ...
      {/* Grid de cards */}
      <section style={{ padding: isMobile ? '24px 24px 80px' : '40px 48px 120px' }}>
        <motion.div
          variants={stagger(0.1)} initial="hidden" whileInView="show" viewport={vp}
          style={{ maxWidth: 1200, margin: '0 auto', display: 'grid', gridTemplateColumns: isMobile ? '1fr' : 'repeat(3, 1fr)', gap: 24 }}
        >
          {services.map(s => (
            <motion.div
      ...
      {/* CTA */}
      <section style={{ background: '#181818', padding: isMobile ? '60px 24px' : '100px 48px', textAlign: 'center', borderTop: '1px solid rgba(255,255,255,0.06)' }}>
