...
export default function HomeV2() {
  ...
  // d() adds the loader offset only on first load; normal delays on navigation
  ...
  return (
    <div style={{ background: bg, color: '#fff', fontFamily: 'Inter, sans-serif' }}>
      ...
      <section ref={heroRef} style={{ position: 'relative', height: '100vh', overflow: 'hidden' }}>
        ...
        <motion.div
          style={{ position: 'relative', zIndex: 1, height: '100%', display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', textAlign: 'center', padding: '0 48px', opacity: heroOpacity }}
        >
          <motion.p
          ...
          <motion.h1
            ...
            style={{ fontFamily: 'Bebas Neue, sans-serif', fontSize: 'clamp(56px, 8vw, 96px)', lineHeight: 1.05, letterSpacing: '0.02em', color: '#fff', maxWidth: '14ch', margin: '0 0 20px' }}
          >
          ...
          <motion.p
            ...
            style={{ fontSize: 16, color: grayLight, marginBottom: 40 }}
          >
          ...
          >
            <Link to="/portafolio" style={{ display: 'inline-block', background: '#fff', color: '#000', fontFamily: 'Inter, sans-serif', fontWeight: 600, fontSize: 13, letterSpacing: '0.1em', padding: '16px 40px', borderRadius: 999, textDecoration: 'none' }}>
              EXPLORAR →
            </Link>
          </motion.div>
        </motion.div>
      ...
      {/* ── SERVICIOS ── */}
      <section style={{ background: bg, padding: '80px 48px 120px', borderTop: '1px solid rgba(255,255,255,0.06)' }}>
        <div style={{ maxWidth: 1200, margin: '0 auto' }}>
          <motion.div variants={stagger(0.12)} initial="hidden" whileInView="show" viewport={vp}
            style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 40, alignItems: 'end', marginBottom: 60 }}>
            <div>
            ...
            <motion.div variants={stagger(0.08)} initial="hidden" whileInView="show" viewport={vp}>
              {serviceItems.map((item, i) => (
                <motion.div key={i} variants={fadeUp}
                  ...
                  whileHover={{ x: 6 }}
                  style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', borderBottom: '1px solid rgba(255,255,255,0.1)', padding: '20px 0', cursor: 'pointer', transition: 'border-color 0.2s' }}
                >
      ...
      <section style={{ position: 'relative', minHeight: '70vh', overflow: 'hidden' }}>
        ...
        <div style={{ position: 'relative', zIndex: 1, padding: '80px 48px', minHeight: '70vh', display: 'flex', flexDirection: 'column', justifyContent: 'flex-end', maxWidth: 1200, margin: '0 auto' }}>
          <motion.div variants={stagger(0.1)} initial="hidden" whileInView="show" viewport={vp} style={{ marginBottom: 48 }}>
            <motion.h2 variants={fadeUp} style={{ fontFamily: 'Bebas Neue, sans-serif', fontSize: 'clamp(36px, 5vw, 56px)', color: '#fff', letterSpacing: '0.02em', lineHeight: 1.1, maxWidth: '18ch' }}>
          ...
          <motion.div variants={stagger(0.12)} initial="hidden" whileInView="show" viewport={vp}
            ...
            {[
              ...
              <motion.div key={c.title} variants={fadeUp} whileHover={{ y: -6, transition: { duration: 0.25 } }}
                style={{ background: 'rgba(255,255,255,0.07)', backdropFilter: 'blur(10px)', border: '1px solid rgba(255,255,255,0.12)', borderRadius: 16, padding: 20, cursor: 'default' }}>
      ...
      {/* ── PROCESO ── */}
      <section style={{ background: bg, padding: '120px 48px', borderTop: '1px solid rgba(255,255,255,0.06)' }}>
        <div style={{ maxWidth: 1200, margin: '0 auto' }}>
          <motion.div variants={stagger(0.1)} initial="hidden" whileInView="show" viewport={vp} style={{ marginBottom: 60 }}>
              ...
              {processSteps.map((step, i) => (
                <motion.div key={i} variants={fadeUp} onClick={() => setActiveStep(i)}
                  ...
                  {activeStep === i ? (
                    ...
                    <motion.span whileHover={{ color: '#fff', x: 4 }} style={{ fontFamily: 'Bebas Neue, sans-serif', fontSize: 20, color: gray, letterSpacing: '0.03em', display: 'block', transition: 'color 0.2s' }}>
                      {step.title}
                    </motion.span>
                  )}
                </motion.div>
      ...
      {/* ── PORTAFOLIO ── */}
      <section style={{ background: bg, padding: '80px 0 120px', borderTop: '1px solid rgba(255,255,255,0.06)' }}>
        <motion.div variants={stagger(0.1)} initial="hidden" whileInView="show" viewport={vp}
          style={{ paddingInline: 48, maxWidth: 1200, margin: '0 auto 48px' }}>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 40, alignItems: 'end' }}>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: 20, alignItems: 'flex-end', textAlign: 'right' }}>
              ...
              <motion.div variants={fadeUp} whileHover={{ scale: 1.05 }} whileTap={{ scale: 0.97 }}>
                <Link to="/portafolio" style={{ display: 'inline-block', border: `1px solid rgba(255,255,255,0.25)`, color: '#fff', fontFamily: 'Inter, sans-serif', fontWeight: 600, fontSize: 12, letterSpacing: '0.12em', padding: '12px 28px', borderRadius: 999, textDecoration: 'none', transition: 'border-color 0.2s' }}>
                  VER PORTAFOLIO →
                </Link>
              </motion.div>
            </div>
        ...
        <div style={{ position: 'relative', display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 20, padding: '0 80px' }}>
          ...
          {portfolioProjects.map((p, i) => {
            ...
            return (
              ...
              >
                <img src={p.img} alt={p.name} style={{ width: '100%', aspectRatio: '5/4', objectFit: 'cover', display: 'block', transition: 'transform 0.5s', transform: isCenter ? 'scale(1.0)' : 'scale(1)' }} />
                {isCenter && (
      ...
      {/* ── CTA FINAL ── */}
      <section style={{ background: bg, padding: '120px 48px', textAlign: 'center', borderTop: '1px solid rgba(255,255,255,0.06)' }}>
        <motion.div variants={stagger(0.15)} initial="hidden" whileInView="show" viewport={vp}>
          <motion.h2 variants={fadeUp} style={{ fontFamily: 'Bebas Neue, sans-serif', fontSize: 'clamp(36px, 6vw, 80px)', color: '#fff', letterSpacing: '0.02em', lineHeight: 1.1, maxWidth: '16ch', margin: '0 auto 48px' }}>
            ¿LISTO PARA CONSTRUIR TU HOGAR SOÑADO? DEJANOS TU CONSULTA.
          </motion.h2>
          <motion.div variants={fadeUp} whileHover={{ scale: 1.05 }} whileTap={{ scale: 0.97 }}>
            <Link to="/contacto" style={{ display: 'inline-block', background: '#fff', color: '#000', fontFamily: 'Inter, sans-serif', fontWeight: 600, fontSize: 13, letterSpacing: '0.1em', padding: '18px 48px', borderRadius: 999, textDecoration: 'none' }}>
              DEJAR UNA CONSULTA
            </Link>
          </motion.div>
        </motion.div>
